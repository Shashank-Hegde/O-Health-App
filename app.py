...
API_KEY = "AIzaSyASUfCPNIKGs4tvsMkStfeW8wpCKqJmZzY"  # Replace with your actual API key
...
if not os.path.exists(BIOBERT_MODEL_DIR):
    raise FileNotFoundError(f"BioBERT model directory '{BIOBERT_MODEL_DIR}' not found after extraction.")
...
symptom_list = [
'runny nose', 'sneezing', 'rash', 'dizziness', 'dizzy', 'weakness', 'loss of appetite', 
    ...
    'cramps', 'ulcers', 'bleeding', 'irritation', 'anxiety', 'depression', 
    'insomnia', 'cancer', 'diabetes', 'hypertension', 'allergies', 'weight loss',
    'weight gain', 'hair loss', 'blurred vision', 'ear pain', 'palpitations', 
    'urinary frequency', 'urinary urgency', 'numbness', 'tingling', 'night sweats', 
    'dry mouth', 'excessive thirst', 'frequent urination', 'acne', 'bruising', 
    'confusion', 'memory loss', 'hoarseness', 'wheezing', 'itchy eyes', 'dry eyes', 
    'difficulty swallowing', 'difficulty sleeping', 'restlessness', 'yellow skin', 
    'yellow eyes', 'bloating', 'gas', 'hiccups', 'indigestion', 'heartburn', 'mouth sores', 
    'nosebleeds', 'ear ringing', 'decreased appetite', 'increased appetite', 
    'feeling full quickly', 'unusual sweating', 'dark urine', 'light-colored stools', 
    'blood in urine', 'blood in stool', 'frequent infections', 'delayed healing', 
    'high temperature', 'high blood pressure', 'low blood pressure', 'thirst', 'dehydration', 
    'skin burn', 'sweating', 'feeling cold', 'feeling hot', 'head pressure', 'double vision', 
    'eye pain', 'eye redness', 'eye discharge', 'ear discharge', 'hearing loss', 
    'ringing in ears', 'balance problems', 'taste changes', 'smell changes', 
    'rapid breathing', 'slow breathing', 'breathing difficulty', 'irregular heartbeat', 
    'chest tightness', 'chest heaviness', 'squeezing chest', 'sharp chest pain', 
    'burning chest pain', 'crushing chest pain', 'tightness in chest', 'lightheadedness',
...
additional_followup_questions = [
    {"hi": "What is your age?", "en": "What is your age?", "category": "age", "symptom": None},
    {"hi": "What is your gender?", "en": "What is your gender?", "category": "gender", "symptom": None},
    {"hi": "Where are you currently located?", "en": "Where are you currently located?", "category": "location", "symptom": None},
    {"hi": "How long have you had these symptoms?", "en": "How long have you had these symptoms?", "category": "duration", "symptom": None},
    {"hi": "Are you experiencing any other symptoms?", "en": "Are you experiencing any other symptoms?", "category": "other_symptoms", "symptom": None}
]
...
def generate_audio_with_api_key(text, api_key, lang='hi-IN'):
    ...
    try:
        ...
        if audio_content:
            ...
            logger.error("No audio content received from the API.")
            return None
...
def determine_followup_questions(initial_symptoms, additional_info, asked_question_categories):
    ...
    Ensures 4-5 symptom-based questions distributed among the symptoms, and up to 3 additional questions.
    """
    followup_questions = []
    ...
    # -------------------- Dynamic Allocation of Symptom-Based Questions -------------------- #

    if initial_symptoms:
        num_symptoms = len(initial_symptoms)
        desired_symptom_min = 4
        desired_symptom_max = 5

        # Calculate base questions per symptom
        base_questions_per_symptom = desired_symptom_min // num_symptoms
        extra_questions = desired_symptom_min % num_symptoms  # Distribute the remainder

        # Shuffle symptoms to randomize the distribution of extra questions
        symptoms_list = list(initial_symptoms)
        random.shuffle(symptoms_list)

        for idx, symptom in enumerate(symptoms_list):
            # Determine the number of questions for this symptom
            num_questions = base_questions_per_symptom + (1 if idx < extra_questions else 0)

            # Retrieve available follow-up questions for this symptom
            symptom_questions = get_followup_questions([symptom])

            # Shuffle to randomize question selection
            random.shuffle(symptom_questions)

            # Select questions ensuring each is from a different category and hasn't been asked yet
            selected_questions = []
            for question in symptom_questions:
                ...
                if len(selected_questions) >= num_questions:
                    break  # Reached the desired number of questions for this symptom

            followup_questions.extend(selected_questions)

        # If still below desired_symptom_max, add more symptom-based questions if available
        current_symptom_questions = [q for q in followup_questions if q['symptom'] is not None]
        if len(current_symptom_questions) < desired_symptom_max:
            remaining_questions = []
            for symptom in symptoms_list:
                remaining_symptom_questions = [q for q in get_followup_questions([symptom]) 
                                               if q['category'] not in asked_categories]
                remaining_questions.extend(remaining_symptom_questions)
            ...
            for question in remaining_questions:
                ...
                current_symptom_questions.append(question)
                asked_categories.add(question['category'])
    else:
        # No initial symptoms
        pass  # Handle cases with no initial symptoms if necessary
    ...
    # Select additional questions up to 3
    selected_additional_questions = []
    for question in missing_additional_info:
        if len(selected_additional_questions) >= 3:
            break
    ...
    # Ensure the total number of questions is between 4 to 8
    total_questions = len(followup_questions)
    if total_questions < 4:
        needed = 4 - total_questions
        ...
        if 'other_symptoms' not in asked_categories:
            other_symptoms_question = {
                "hi": "Are you experiencing any other symptoms?",
    ...
    # Limit to maximum of 8 questions
    if total_questions > 8:
        followup_questions = followup_questions[:8]
...
def extract_all_symptoms(conversation_history, symptom_list, symptom_to_canonical):
    ...
    matched_symptoms = exact_symptoms.union(canonical_symptoms)

    logger.info(f"Final Exact Symptoms: {matched_symptoms}")
...
def handle_yes_no_response(question, response):
    ...
    matched_symptoms = session.get('matched_symptoms', set())

    if is_affirmative and question['symptom']:
        matched_symptoms.add(question['symptom'].lower())  # Ensure lowercase
        logger.info(f"Added symptom '{question['symptom']}' based on affirmative response.")
    elif is_negative and question['symptom']:
        if question['symptom'].lower() in matched_symptoms:
            matched_symptoms.remove(question['symptom'].lower())
...
def index():
    ...
    if 'matched_symptoms' not in session:
        session['matched_symptoms'] = set()
    if 'additional_info' not in session:
        session['additional_info'] = {}
    if 'asked_question_categories' not in session:
        session['asked_question_categories'] = set()
    if 'followup_questions' not in session:
        session['followup_questions'] = []
    if 'current_question_index' not in session:
        session['current_question_index'] = 0
    if 'report_generated' not in session:
    ...
    if request.method == 'POST':
        ...
        if user_input == '':
            ...
            # Extract symptoms and additional info
            ...
            followup_questions = determine_followup_questions(
                matched_symptoms,
                additional_info,
                session.get('asked_question_categories', set())
            )
...
def ask_questions():
    ...
    question_text = current_question['en']

    if request.method == 'POST':
        response = request.form.get('response', '').strip()
        if response == '':
            error = "Please provide an answer."
            return render_template('ask_questions.html', question=question_text, error=error)
        else:
            ...
            # Update asked question categories
            asked_categories = session.get('asked_question_categories', set())
            asked_categories.add(current_question['category'])
            session['asked_question_categories'] = asked_categories

            # Handle yes/no responses if applicable
    ...
    return render_template('ask_questions.html', question=question_text)

@app.route('/report')
def report():
    if session.get('report_generated', False):
    ...
    if translated_cause != "आपके लक्षणों के आधार पर कोई संभावित कारण नहीं पाया गया।":
        message_hindi = f"आपके लक्षण: {translated_symptoms}. इन लक्षणों के कारण, संभावित कारण यह हैं: {translated_cause}. हम आपको सबसे उपयुक्त {specialist} डॉक्टर से तुरंत जोड़ रहे हैं।"
    else:
        message_hindi = f"{translated_cause} हम आपकी मदद के लिए एक डॉक्टर से संपर्क कर रहे हैं।"

    # Generate the audio in Hindi
    audio_bytes = generate_audio_with_api_key(message_hindi, API_KEY, lang='hi-IN')
    ...
    return render_template('report.html', symptoms=exact_symptoms, additional_info=additional_info,
                           possible_cause=possible_cause, specialist=specialist, conversation_history=conversation_history,
                           audio_src=audio_src)
